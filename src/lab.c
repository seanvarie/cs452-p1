...
int change_dir(char **dir)
{
    ...
    if(directory == NULL){//try getting home from environment
        directory = getenv(HOME_ENV_VARIABLE);
    }
    if(directory == NULL){//try getting home from uid
...
struct jobs * evaluate_background_jobs(struct jobs * listHead, bool printAll)
{
  ...
  while(NULL != current){
    if(-1 == waitpid(current->processID, NULL, WNOHANG)){
      ...
      if(NULL == previous){//retVal should be the same as current if this is true
        retVal = retVal->next;
        current->next = NULL;
